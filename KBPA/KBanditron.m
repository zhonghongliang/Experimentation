...
model = 'laplace';
%model = 'linear';
parameter = 10;% 1 for segment; 10 for pendigits
for i=1:n
    i
    t0 = clock;
	xt = X(i,:);
	...
	[dummy2,ty] = max(dummy);
	BF = (ty == Y(i));
	lt = max( 1 + score(hy) - score(Y(i)),0);
    if ty == Y(i)
        Container{ty,1}.x = [Container{ty,1}.x; xt];
        Container{ty,1}.alpha = [Container{ty,1}.alpha;+1/P(ty)];
    end
    Container{hy,1}.x = [Container{hy,1}.x; xt];
    Container{hy,1}.alpha = [Container{hy,1}.alpha;-1];
